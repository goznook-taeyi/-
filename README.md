...
서버를 실행한 뒤 브라우저에서 **http://127.0.0.1:8756/** 을 열면 숏폼 솔팅기가 나옵니다.

1. 유튜브 URL을 붙여넣고 **목록에 추가** (여러 개면 줄바꿈으로 구분)
2. 마음에 드는 영상은 **골라내기** 버튼으로 표시 — "골라낸 것만" 탭으로 모아볼 수 있습니다
3. 각 영상 **우측의 ⬇ 다운로드 버튼**을 누르면 바로 다운로드되고 버튼에 진행률이 표시됩니다
4. **⬇ 골라낸 영상 모두 다운로드** 버튼으로 골라낸 영상을 일괄 다운로드할 수 있습니다

목록은 브라우저(localStorage)에 저장되므로 새로고침해도 유지됩니다.

## 외부 프로그램 연동 (예: 숏폼솔팅기)

> 골라낸 영상 **옆에 다운로드 버튼을 다는 방법**(웹/tkinter 스니펫, 클로드에게 맡기는 프롬프트 포함)은 [INTEGRATION.md](INTEGRATION.md)에 있습니다.
